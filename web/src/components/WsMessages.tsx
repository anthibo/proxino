...
export function WsMessages({ flowId }: { flowId: string }) {
  const msgs = useStore((s) => s.wsMessages[flowId] ?? EMPTY);
  ...
  useEffect(() => {
    ...
    try { listRef.current.scrollTop = listRef.current.scrollHeight; } catch { /* ignore (e.g. non-writable in tests) */ }
  }, [msgs.length, follow]);
  const rows = useMemo(() => msgs.filter((m) => {
    if (dir !== "all" && m.dir !== dir) return false;
    if (!q) return true;
    const needle = q.toLowerCase();
    return (m.text ?? "").toLowerCase().includes(needle) || (m.pretty ?? "").toLowerCase().includes(needle);
  }), [msgs, dir, q]);
  return (
    <div className="wsm">
      ...
      {msgs.length === 0 && <div className="wsm-empty">No frames yet</div>}
    </div>
  );
}
